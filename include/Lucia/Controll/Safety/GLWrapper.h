...
namespace Lucia
{
    ...
    void Error_GLWrapper(Error* lucy,Context::Window* w)
    {
        lucy->RenderCriticalErrors = [w](Error::Function* fn)
        {
            float width = 800.0f;
            float height = 600.0f;
            int fontSize =25;
            ...
            text.generate();
            auto ortho = Maths::Ortho(0.0f, width,0.0f,height, 0.01f, 100.0f);
