...
namespace Graphics
{
    ...
    bool Text::Font::subfont::generate()
    {
        ...
        for (GLubyte  c=0;c < 128;c++)
        {
            if (!FT_Load_Char(face,c,FT_LOAD_RENDER))
            {
                ...
                Chars.insert({c,cn});
                maxheight = std::max(maxheight,cn->size.y);
            }
        }
        Finalized = true;
    }
    ...
    void Text::setProjection(glm::mat4* p)
    {
        projection = p;
    }
    ...
    std::shared_ptr<Graphics::Canvas> Text::render(string text,bool sendmatrix)
    {
        ...
        if (projection == nullptr)
        {
            ...
            auto d = glm::ortho(0.0f,(float)dim[2],0.0f,(float)dim[3],-100.0f,100.0f);
            glUniformMatrix4fv(glGetUniformLocation(Graphics::textShader->programID, "projection"),
            1,GL_FALSE,glm::value_ptr(d));
        }else
        {
            glUniformMatrix4fv(glGetUniformLocation(Graphics::textShader->programID, "projection"),
